...
import Heading from '@/app/components/atoms/commons/Heading';
import Paragraph from '@/app/components/atoms/commons/Paragraph';
import BadgeArrow from '@/app/components/atoms/commons/BadgeArrow';
...
const BannerDescription: React.FC<BannerDescriptionProps> = ({ title, description }) => {
  ...
  return (
    <div className="flex-1 flex flex-col gap-4">
      ...
      <Paragraph className={`hidden md:block font-bold text-lg text-left underline text-white transition-all duration-500 delay-300 ${isVisible.description ? 'translate-y-0 opacity-100' : 'translate-y-5 opacity-0'}`}>개발자가 필요하신가요?</Paragraph>
    </div>
  );
};
